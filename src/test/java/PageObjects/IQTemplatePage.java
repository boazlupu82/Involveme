...
public class IQTemplatePage extends BasePage{
	...
	public void chooseFirstAns() {
		ExpectedCondition<Boolean> elementTextContainsString = arg0 -> textQuestion2.getText().contains("Complete the sequence");
		wait.until(elementTextContainsString);
		click(firstAns);
	}
	@Step("2Th answer is:{0}")
	public void chooseScndAns(String answer) {
		ExpectedCondition<Boolean> elementTextContainsString = arg0 -> textQuestion2.getText().contains("What has similar meaning");
		wait.until(elementTextContainsString);
	...
	public void chooseThirdAns(String answer) {
		ExpectedCondition<Boolean> elementTextContainsString = arg0 -> textQuestion2.getText().contains("Find the analogy");
		wait.until(elementTextContainsString);
		List<WebElement> sal=secondAnsList;
		for(int i=0; i<sal.size();i++) {
			if(getText(sal.get(i)).equalsIgnoreCase(answer))
	...
	public void chooseForthAns(String answer) {
		ExpectedCondition<Boolean> elementTextContainsString = arg0 -> textQuestion2.getText().contains("Continue the sequence");
		wait.until(elementTextContainsString);
		List<WebElement> sal=secondAnsList;
		for(int i=0; i<sal.size();i++) {
			if(getText(sal.get(i)).equalsIgnoreCase(answer))
	...
	public void chooseFifthAns(String answer) {
		ExpectedCondition<Boolean> elementTextContainsString = arg0 -> textQuestion2.getText().contains("Find the odd one");
		wait.until(elementTextContainsString);
		List<WebElement> sal=secondAnsList;
		for(int i=0; i<sal.size();i++) {
			if(getText(sal.get(i)).equalsIgnoreCase(answer))
